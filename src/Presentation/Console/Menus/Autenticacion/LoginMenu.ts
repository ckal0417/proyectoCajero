...
function normalizeModule<T>(mod: T & { default?: T }): T {
    return (mod && typeof mod === "object" && "default" in mod ? mod.default as T : mod) as T;
}
...
export class LoginMenu {
    ...
    public iniciar(
        ...
        void (async () => {
            const reactModule = normalizeModule(await import("react")) as typeof import("react");
            const inkModule = normalizeModule(await import("ink")) as typeof import("ink");

            const { render, Box, Text, useInput } = inkModule;
            ...
            const Alert = ({ title, children }: { title: string; children?: any }) => {
                return React.createElement(
                    Box,
                    {
                        flexDirection: "column",
            ...
            const LoginInkScreen = ({ onLoginSuccess }: LoginInkScreenProps) => {
            ...
            const inkInstance = render(
                React.createElement(LoginInkScreen, {
                    onLoginSuccess: (sesion: SesionAutenticada) => {
                        inkInstance.unmount();
